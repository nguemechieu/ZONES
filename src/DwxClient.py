...
class DwxClient:
    ...
    """ Constructor for the Dwx client object """
    ...
    def __init__(self, event_handler=None, metatrader_dir_path='',
        ...
        self.path_bar_data_stored = join(metatrader_dir_path, 'DWX', 'DWX_Bar_Data_Stored.txt')
        ...
        self.server_status = {'server_status': 'OK', 'message': 'server is up and running',
                              'info': 'DWX server is up and running.', 'orders': [],
                              'orders_stored': [], 'messages': [], 'messages_stored': [],
                              'commands': [], 'commands_stored': [],
                              'server_time': datetime.now().strftime('%Y-%m-%d %H:%M: %S'),
                              'market_data': [], 'bar_data': [], 'historic_data': []}
        ...
        self.market_data = pd.DataFrame(columns=['symbol', 'bid', 'ask', 'last_price', 'tick_value'])
        self.bar_data = pd.DataFrame(columns=['symbol', 'time', 'open', 'high', 'low', 'close', 'tick_volume'])
        self.historic_data = pd.DataFrame(
            columns=['symbol', 'time', 'price', 'lots', 'order_id', 'trade_id', 'type', 'open_price', 'close_price'])
        self.historic_trades = pd.DataFrame(
            columns=['symbol', 'time', 'order_id', 'trade_id', 'type', 'open_price', 'close_price', 'quantity', 'price',
                     ''])
        ...
        self.server_status['status'] = 'LOADING...'
        # no need to wait.
        if self.event_handler is None:
            self.server_status['status'] = 'READY'
            self.start()
    ...
    def start(self):
        ...
        self.server_status['status'] = 'Tracking Trades...'
    ...
    def try_read_file(self, file_path):
        ...
        # can happen if mql writes to the file. don't print anything here.
            ...
            self.server_status['status'] = 'ERROR'
            self.server_status['info'] = 'Error reading file: {}'.format(file_path)
            return ''
    ...
    def try_remove_file(self, file_path):
        ...
        for _ in range(10):
            try:
                ...
                print_exc()
                self.server_status['status'] = 'ERROR'
                self.server_status['info'] = 'Error removing file: {}'.format(file_path)
    ...
    def check_open_orders(self):
        ...
        while self.ACTIVE:
            ...
            if text is None:
              ...
              print('Error reading file: {}'.format(self.path_orders))
              continue
            ...
            for order_id, order in self.open_orders.items():
                ...
                if order_id not in data['orders'].keys():
                    ...
                    if self.verbose:
                        ...
                        self.server_status['info'] = 'Order removed: {}'.format(order)
            ...
            for order_id, order in data['orders'].items():
                if order_id not in self.open_orders:
                    ...
                    if self.verbose:
                        ...
                        self.server_status['info'] = 'New order: {}'.format(order)
    ...
    def check_messages(self):
        ...
        while self.ACTIVE:
            ...
            if text is None:
                print('Error reading file: {}'.format(self.path_messages))
                continue
            ...
            for millis, message in sorted(data.items()):
                if int(millis) > self._last_messages_millis:
                    ...
                    print(message)
                    self.server_status['INFO'] = message
    ...
    def check_market_data(self):
        ...
        while self.ACTIVE:
            ...
            if text is None:
                print('Error reading file:{}'.format(self.path_market_data))
                return
            ...
            if self.event_handler is not None:
                for symbol in data.keys():
                    if symbol not in self._last_market_data \
            ...
            self.server_status['market_data'] = self.market_data
    ...
    def check_bar_data(self):
        ...
        while self.ACTIVE:
            ...
            if text is None:
                print('Error reading file: {}'.format(self.path_bar_data))
                return
            ...
            if self.event_handler is not None:
                for st in data.keys():
                    if st not in self._last_bar_data or data[st] != self._last_bar_data[st]:
                        ...
                        self.bar_data['time'] = [data[st]['time']]
                        self.bar_data['open'] = [data[st]['open']]
                        self.bar_data['high'] = [data[st]['high']]
                        self.bar_data['low'] = [data[st]['low']]
                        self.bar_data['close'] = [data[st]['close']]
                        self.bar_data['tick_volume'] = [data[st]['tick_volume']]

                        self.server_status['bar_data'] = data
            self._last_bar_data = data
            with open(self.path_bar_data_stored, 'w') as f:
                f.write(json.dumps(data))
    ...
    def check_historic_data(self):
        ...
        while self.ACTIVE:
            ...
            if not self.START:
                self.server_status['INFO'] = 'Restarting historic data check...'
                continue

            text = self.try_read_file(self.path_historic_data)
            if text is None:
                print('Error reading file: {}'.format(self.path_historic_data))
                return
            if len(text.strip()) <3 and text == self._last_historic_data_str:
                return
            ...
            try :
             ...
             self.server_status['historic_data'] = self.historic_data
            ...
            # also check historic trades in the same thread.
             ...
             if text is None or text == self._last_historic_trades_str:
                print('Error reading file: {}'.format(self.path_historic_trades))
                continue
             if len(text.strip()) > 0 and text != self._last_historic_trades_str:
                ...
                self.server_status['historic_trades'] = self.historic_trades
                ...
                self.server_status['INFO'] = 'Restarting historic trades check...'
                continue
    ...
    def load_orders(self):
        ...
        if text is None:
            print('Error reading file %s' % self.path_orders_stored)
            self.server_status['INFO'] = 'Restarting orders check...'
            return
        if len(text) > 0:
            self._last_open_orders_str = text
            data = json.loads(text)
            self.account_info = data['account_info']
            self.server_status['account_info'] = self.account_info
            self.open_orders = data['orders']
            self.account_info['orders'] = self.open_orders
            self.server_status['orders'] = self.open_orders
            ...
            for order_id, order in self.open_orders.items():
                ...
                order_info = self.account_info['orders'][order_id]
                if order_info['type'] == 'buy':
                    self.account_info['profit'] = order_info['pnl']
                    
                    self.server_status['profit'] = self.account_info['profit']
                    self.server_status['INFO'] = 'Restarting orders check...'
                elif order_info['type'] == 'sell':

                    self.account_info['profit'] = [order_info['pnl'][-1]]
    ...
    def subscribe_symbols(self, symbols):
        """Sends a SUBSCRIBE_SYMBOLS command to subscribe to symbols.  """
            ...
            for i in range(self.num_command_files):
                ...
                if not exists(file_path):
                    try:
                        ...
                        self.reset_command_ids()
                        sleep(self.sleep_delay)
                        self.server_status['status'] = 'error - command file already exists '
                        self.server_status['message'] = f'Command file {file_path} already exists.'
